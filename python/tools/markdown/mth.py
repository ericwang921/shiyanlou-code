...
class ArgsParse:
    ...
    def __init__(self):
        ...
        # 一个是选项解析结果，另一个是其余参数
        options, _ = getopt.getopt(sys.argv[1:], 'hf:d:S:F:t:', ['help'])
    ...
    def args_parse(self):
        ...
        self.table_of_content = self.options_dict.get('-t')
...
class Markdown2Html:
    ...
    def switch(self):
        ...
        if args.table_of_content == 'T':
            markdown_text = f'[TOC]\n{markdown_text}'

        value = markdown.markdown(markdown_text, output_format='html5', extensions=[TocExtension(baselevel=1)])
        # 为 HTML 文本添加 head 标签
        ...
        # 将 HTML 文本存入文件
        with open(html_file_route, 'w', encoding='utf-8') as f:
